...
def get_table_save_reservation():
    ...
    if session['user_id']:
        ...
        guest_user = User(session['first_name'], session['last_name'], session['email'], session['phone_number'],
                          session['user_type_id'])
        user_id = guest_user.save_guest_info()

    new_reservation = Reservation(session['date'], session['start_time'], session['end_time'],
                                  session['number_of_people'], user_id)
